...
export function editSubcommandBuilder(sub: SlashCommandSubcommandBuilder) {
	return sub
		.setName("edit")
		.setDescription(
			"Edit a registered Minecraft server's plugin & proxy configuration",
		);
}
			...
			fields: [
				{
					...
					required: true,
					defaultValue: current.pluginDir,
				},
				{
					id: "ipcSocket",
					...
					required: false,
					defaultValue: current.ipcSocket ?? "",
				},
			],
		},
		{
			label: "Loader",
			description: [
				`Server software. Leave the menu untouched to keep ${inlineCode(current.loaderType || "unset")}.`,
			...
			fields: [
				{
					...
					placeholder: `Current: ${current.loaderType || "unset"} — leave to keep`,
					selectOptions: KNOWN_LOADERS.map((l) => ({
						label: l,
						value: l,
						description:
							l === current.loaderType ? "current" : undefined,
					})),
					required: false,
				},
			],
			...
			fields: [
				{
					...
					label: "Proxied",
					type: "select" as const,
					placeholder: `Current: ${current.proxy.enabled ? "enabled" : "disabled"} — leave to keep`,
					selectOptions: [
						{
							label: "Enabled",
							value: "true",
				...
				},
				{
					...
					type: "select" as const,
					placeholder: `Current: ${current.proxy.forwarding} — leave to keep`,
					selectOptions: FORWARDING_MODES.map((m) => ({
						label: m,
						value: m,
						description:
							m === current.proxy.forwarding
								? "current"
								: undefined,
					})),
					required: false,
				},
				{
					...
					required: true,
					defaultValue: current.proxy.host,
				},
				{
					id: "forwardingSecret",
					...
					required: false,
					defaultValue: current.proxy.forwardingSecret ?? "",
				},
			],
			validate: (values) => {
...
export async function editHandler(
	...
	const { config: current, invalidReason } = readConfig(dbServer.config);
	...
	const loaderType = phaseValues[1]!.loaderType || current.loaderType;
	const enabledPick = phaseValues[2]!.enabled;
	const forwarding = (phaseValues[2]!.forwarding ||
		current.proxy.forwarding) as MinecraftConfig["proxy"]["forwarding"];
	const host = phaseValues[2]!.host?.trim() || current.proxy.host;
	...
	const changes = diffLines(current, validated.config);
