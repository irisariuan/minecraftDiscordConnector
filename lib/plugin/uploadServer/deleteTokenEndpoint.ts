...
export function setupDeleteTokenEndpoint(uploadServer: UploadServer) {
	return (req: Request, res: Response) => {
		...
		const dispose = req.query.dispose === "true" || req.body?.dispose === true;
		...
		if (!uploadServer.hasToken(token)) {
			return res.status(404).send("Token not found");
		}

		// Check if token is still active
		const isActive = uploadServer.hasActiveToken(token, null);
		...
		if (dispose) {
			...
			if (isActive) {
				uploadServer.activeTokens.delete(token);
				uploadServer.emit("tokenDeleted", token);
				uploadServer.checkTokens();
				return res.status(200).send({
					success: true,
